...
class ViewController: UITableViewController, NSFetchedResultsControllerDelegate {
    ...
    func changeFilter() {
        ...
        ac.addAction(UIAlertAction(title: "Show only fixes", style: .default, handler: { [unowned self] alert in
            self.commitPredicate = NSPredicate(format: "message CONTAINS[c] 'fix'")
            self.loadSavedData()
        }))
        ac.addAction(UIAlertAction(title: "Ignore Pull Requests", style: .default, handler: { [unowned self] alert in
    ...
    func saveContext() {
        if container.viewContext.hasChanges {
            do {
                ...
                print("An error occurred while saveing: \(error)")
            }
        }
    }
    // MARK: Core data
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if let vc = storyboard?.instantiateViewController(withIdentifier: "Detail") as? DetailViewController {
            let author = fetchedResultsController.object(at: indexPath).author.name
