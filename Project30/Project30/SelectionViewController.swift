...
//	var viewControllers = [UIViewController]() // create a cache of the detail view controllers for faster loading
    ...
    func convertImages() {
        ...
        // find the image for this cell, and load its thumbnail
            ...
            let newFileName = getDocumentsDirectory().appendingPathComponent(imageRootName)
            
            if !FileManager.default.fileExists(atPath: newFileName.path) {
                ...
                FileManager.default.createFile(atPath: newFileName.path, contents: rounded.pngData())
            }
        }
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		let imageRootName = currentImage.replacingOccurrences(of: "Large", with: "Thumb")
        let path = getDocumentsDirectory().appendingPathComponent(imageRootName).path
		let original = UIImage(contentsOfFile: path)!
